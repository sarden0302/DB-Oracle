...
/*
INDEX(색인)
- SQL 구문 중 SELECT 처리 속도를 향상 시키기 위해 COLUMN에 대하여 생성하는 객체

- INDEX 내부 구조는 B*트리 형식으로 되어있음
※ B*트리 : 하나의 노드에 많은 수의 정보를 가질 수 있음

[INDEX의 장점]
1. 이진 트리 형식으로 구성되어 자동 정렬 및 검색 속도 증가
2. 조회 시 테이블의 전체 내용을 확인하며 조회하는 것이 아닌 인덱스가 지정된 
  COLUMN만을 이용하여 조회하기 떄문에 시스템의 부하가 낮아짐

[INDEX의 단점]
1. 데이터 변경(INSERT, UPDATE, DELETE) 작업 시 이진 트리 구조에 변형이 일어남
    -> DML 작업이 빈번한 경우 시스템 부하가 늘어 성능이 저하됨
2. 메모리 소비 : 인덱스도 하나의 객체이다 보니 별도 저장공간이 필요
3. 인덱스 생성 시간이 필요함
...
인덱스 사용 방법 ! -> WHERE 절에 INDEX가 지정된 컬럼 언급하기
SELECT * FROM employee
WHERE emp_id != 0;    -- INDEX 사용해서 직원번호 0과 일치하지 않는 사람 모두 조회
*/